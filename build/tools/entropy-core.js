...
const EntropyCore = (() => {
  ...
  /* The smallest without-replacement draw that reaches a BIP39 entropy
     target. Keep this derived from cardEntropy rather than repeating 25 and
     58 as policy constants: the progress meter and the acceptance boundary
     must always be answering the same mathematical question. Two complete
     shuffles are already more than any BIP39 phrase can require. */
  const cardsNeeded = targetBits => {
    if (!Number.isFinite(targetBits) || targetBits <= 0) {
      throw new Error('card entropy target must be a positive number');
    }
    for (let drawn = 1; drawn <= 104; drawn++) {
      if (cardEntropy(drawn) >= targetBits) return drawn;
    }
    throw new Error('card entropy target exceeds two shuffled decks');
  };

  const HASHED_CARD_COUNTS = Object.freeze(Object.fromEntries(
    [12, 24].map(words => [words, cardsNeeded(words * 32 / 3)])
  ));
  ...
  const METHODS = {
    ...
    cards: {
      ...
      /* 25 cards carry 132.4 bits and 58 carry 259.3; one short of either and
         the seed would be padded with nothing. Derived from cardEntropy above
         rather than duplicated as rounded policy constants. */
      counts: HASHED_CARD_COUNTS,
      extra: true,
      most: 104,
  ...
  return {
    ...
    cardBits, cardEntropy, cardsNeeded, cardsLeft, repeatedCard, cardAliasAmbiguity, colemanCardTranscript,
