...
${FONTS.map(embedFont).join('\n')}
  ...
  :root {
    ...
    --surface: #242220;
    --line: #6e695e;
    /* The resting edge every control in the workbench sits at. A token rather
       than the literal repeated per control, so a method pad that only becomes
       visible when the source changes inherits it without being listed
       anywhere. Resting only: hover, pressed, disabled, warning and error all
       set their own border-color and keep winning. */
    --edge-rest: rgba(232, 214, 181, 0.34);
    --success: #35b48a;
    --danger: #d65e40;
    /* The embedded Jost and Open Sans subsets do not contain the four card
       suits. Name local symbol faces before the generic fallback so the
       keypad and recorded deal remain unambiguous on Windows, macOS and
       Linux without downloading another font. */
    --workshop-sans: "Open Sans", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, "Segoe UI Symbol", "Apple Symbols", "Noto Sans Symbols", "DejaVu Sans", sans-serif;
    --workshop-mono: ui-monospace, SFMono-Regular, Menlo, Consolas, "Segoe UI Symbol", "Apple Symbols", "Noto Sans Symbols", "DejaVu Sans Mono", "DejaVu Sans", monospace;
  }
  ...
  * { box-sizing: border-box; }
  ...
  .seg button {
    ...
    border: 1px solid var(--edge-rest); border-radius: 10px;
  ...
  .seg button:disabled:hover { border-color: var(--edge-rest); background: rgba(255, 255, 255, 0.03); }
  ...
  .export-card.is-private {
    border-color: rgba(229, 105, 77, 0.74);
  ...
  .export-card.is-watch {
    border-color: rgba(65, 205, 158, 0.84);
  ...
  .export-card-icon {
    display: grid; flex: 0 0 36px; width: 36px; height: 36px; place-items: center;
    border: 1px solid currentColor; border-radius: 10px;
    box-shadow: inset 0 1px 0 rgba(255, 255, 255, 0.06);
  }
  .export-card-icon svg { width: 19px; height: 19px; }
  .is-private .export-card-icon { color: #ffad9c; background: rgba(214, 94, 64, 0.18); border-color: rgba(255, 157, 138, 0.52); }
  .is-watch .export-card-icon { color: #9af0d3; background: rgba(53, 180, 138, 0.18); border-color: rgba(139, 227, 198, 0.56); }
  /* An eyebrow, not a pill. As a pill its 8px of side padding put its letters
     8px right of the heading directly beneath it: the box lined up, the text
     did not, and the eye reads the text -- so the two lines stepped. Without
     the padding they share one left edge and read as one block. The colour is
     unchanged and still on the text, alongside the icon and the card's own
     border, so nothing that told the two cards apart is lost. */
  .export-tag {
    display: block; width: fit-content; margin-bottom: 5px;
    color: var(--muted);
    font-size: 0.78rem; font-weight: 800; line-height: 1.25; letter-spacing: 0.075em; text-transform: uppercase;
  }
  .is-private .export-tag { color: #ffad9c; }
  .is-watch .export-tag { color: #9af0d3; }
  /* 700, not 800. The section headings around these -- "Recovery words",
     "Addresses", "Export files" -- are Jost 500 at 16px, so the page uses Jost
     as a light geometric face; at 800 its bowls thicken and it stops looking
     like the same typeface. The emphasis comes from the size instead, which
     still puts the card title above the heading that contains it.

     Jost rather than the eyebrow's Open Sans: every heading on the page is
     Jost, and the eyebrow is a label like the Download: caption below it, not
     a heading. The two faces here are the system, not a mismatch. */
  .export-card h4 { margin: 0; color: #fff; font-size: 1.12rem; font-weight: 700; }
  ...
  .export-button:hover:not(:disabled) { transform: translateY(-1px); color: #fff; border-color: rgba(255, 138, 0, 0.56); }
  .export-button:disabled { opacity: 0.55; cursor: wait; }
  .is-private .export-button { color: #ffad9c; border-color: rgba(229, 105, 77, 0.62); background: rgba(214, 94, 64, 0.1); }
  .is-private .export-button:hover:not(:disabled) { border-color: rgba(255, 157, 138, 0.72); }
  .is-watch .export-button { color: #9af0d3; border-color: rgba(65, 205, 158, 0.68); background: rgba(53, 180, 138, 0.13); }
  .is-watch .export-button:hover:not(:disabled) { border-color: rgba(154, 240, 211, 0.9); background: rgba(53, 180, 138, 0.19); }
  ...
  .key {
    ...
    border: 1px solid var(--edge-rest); border-radius: 11px;
  ...
  .key-tool {
    ...
    background: rgba(255, 255, 255, 0.03);
    border: 1px solid var(--edge-rest); border-radius: 9px;
  ...
  .key[data-suit] {
    padding-left: 4px; padding-right: 4px; font-family: var(--workshop-sans);
  ...
  .pad[data-method="cards"], .pad[data-method="cardscoleman"], .pad[data-method="cardbits"] {
    /* 62px, not 58. At 58 auto-fit fitted fourteen tracks across the 934px
       pad and there are only thirteen ranks, so the row ran out one key early
       and left a column of empty space on the right. The pad is never wider
       than 934, and a 62px minimum admits at most floor(942 / 70) = 13 tracks
       there, so the count lands exactly on the ranks and 1fr spends the rest
       on them: 64.5px per key rather than 59.3px.

       Raising the minimum rather than writing repeat(13, ...): thirteen fixed
       columns hold at full width but keep dividing as the pad narrows, and at
       830px they gave a 47.1px key with QUEEN painting 12.3px past its own
       edge -- the same overflow the suit labels were once fixed for. 58px was
       never arbitrary; it is about what the longest rank name needs. This way
       the count falls as the pad narrows and the keys stay legible. */
  ...
  .dl {
    ...
    /* font-family and cursor are inherited by an <a> but not by a <button>.
       Stating them here lets the acknowledgement button wear this same skin
       instead of a near-copy of it -- see .beta-disclaimer-actions. Both are
       no-ops for the download link. */
  ...
  /* On the skin itself rather than on either button, so the lift and the
     sweep cannot survive in one place after being removed from the other. */
  ...
  .address-match {
    ...
    /* A flat tint of #DEC93C, like the export cards, rather than the gradient
       that was here. The colour is now carried by the border alone.

       A tinted fill was the wrong lever twice over. Thinned over this
       near-black ground a yellow near 52deg desaturates toward olive -- the
       green cast was the ground showing through, not the swatch -- and any
       fill strong enough to read as a colour also puts a wash under the body
       text and drags the quiet note toward the contrast floor. A border owes
       nothing to the text it surrounds, so it can hold the full #EDC873 while
       the panel keeps the same grey as the address boxes above it. */
  ...
  /* No control beside the field: the check runs on input, so there is nothing
     ...
     field. */
  /* Even gaps above and below. The field inherited .hint's 7px underneath it
     from the status line and nothing on top, so it sat against the
     description and floated away from its own result. */
  .address-match input[type="text"] { margin-top: 10px; }
  ...
  .address-match-local svg { flex: 0 0 auto; width: 12px; height: 12px; color: var(--muted); }
  ...
  /* The 26px every other disclosure needs separates it from a paragraph. This
     one follows the export cards, which already end in their own padding, so
     the two gaps stacked and it drifted away from the section it belongs to. */
  details.raw-entropy { margin-top: 14px; }
  ...
  /* Border and text only. The surface stays the one every badge in this row
     shares -- a badge that carried its own fill sat forward of the two beside
     it and read as the important one, which is backwards: the self-test is
     the badge that matters. Colour is enough to say this one is a label
     rather than a check result. */
  ...
  /* Padding is even on all four sides now that the icon leads the card on its
     own line rather than sitting in an 86px left gutter, and the ::before bar
     that used to sit along the top edge is gone: a 3px orange rule above a 1px
     orange border read as one heavier side rather than as a deliberate accent. */
  .beta-disclaimer-card {
    position: relative; width: min(100%, 590px); padding: 30px 34px 28px;
  ...
  .beta-disclaimer-icon { display: flex; justify-content: center; margin-bottom: 14px; }
  /* The shared .security-icon is absolutely positioned for the in-page brief,
     which is not what a centred row wants. */
  .beta-disclaimer-card .security-icon { position: static; width: 38px; height: 38px; }
  .beta-disclaimer-card h2 { margin: 0 0 22px; color: #fff; font-size: 1.65rem; text-align: center; }
  /* Heading centred, the warnings themselves left-aligned: centred prose is
     harder to track back to the start of each line, and these two paragraphs
     are the part that has to be read rather than glanced at. */
  .beta-disclaimer-body { display: grid; gap: 18px; text-align: left; }
  .beta-disclaimer-card p { margin: 0; line-height: 1.65; }
  .beta-disclaimer-card p strong { color: #ffad4c; }
  .beta-disclaimer-divider { height: 1px; background: rgba(255, 244, 233, .13); }
  /* Keeps its red, loses its box. With a divider now separating the two
     warnings, the outlined panel made the card two frames deep. */
  ...
  /* Wears .dl-frame and .dl -- the download action's own skin, not a copy of
     it -- so the two buttons cannot drift as the site changes. Only the width
     is set here; everything visible comes from the shared rules above. */
  .beta-disclaimer-actions { display: flex; justify-content: center; margin-top: 26px; }
  .beta-disclaimer-actions .dl-frame { width: min(100%, 236px); }
  @media (prefers-reduced-motion: reduce) {
    .beta-disclaimer { transition: none; }
  }
  ...
  /* Same treatment as .results, which sits inside it and was built the same
     way -- fixing one and not the other would have left the two frames lit
     differently. The drop shadow and the orange glow stay; they are cast
     outside the box and do nothing to the border. */
  .workbench {
    position: relative; padding: 16px 24px 24px; border: 1px solid rgba(255,255,255,.12);
    border-radius: 22px; background: rgba(255,255,255,.032);
    box-shadow: 0 26px 68px rgba(0,0,0,.26), 0 -1px 32px rgba(255,138,0,.035);
  }
  ...
  .workbench legend::before {
    ...
    /* vertical-align: middle aligns the badge's centre to the baseline plus half
       the x-height,
       which leaves it sitting (cap - x)/2 below the centre of the heading's cap
       band -- measured at 2.32px for Jost at 16px.

       rem, not em: em on a pseudo-element resolves against its own font-size,
       and the shorthand below sets that to 0.82rem, which gave 1.90px instead.
       rem tracks the root, so the correction scales with the legend's font
       under browser zoom rather than being frozen in pixels. The entry step's
       legend is 1.07rem and strictly wants 0.155rem; the 0.16px difference is
       not worth a second rule. Paint only -- the line box is unchanged, so
       nothing reflows. */
    position: relative; top: -0.145rem;
    margin-right: 13px;
  ...
  /* Flat fill, no inset highlight. Both were doing the same thing to the same
     ...
     so top and left read heavier than right and bottom. The border is 1px on
     all four sides and now looks it. The flat value is the midpoint of the
     gradient it replaces, so the panel keeps the weight it had. */
  .workbench .results { margin-top: 32px; padding: 28px; border: 1px solid rgba(243,234,220,.26); border-radius: 16px; background: rgba(243,234,220,.05); }
  ...
  @media (max-width: 620px) {
    ...
    .beta-disclaimer-card { padding: 24px 20px 22px; border-radius: 15px; }
    .beta-disclaimer-card .security-icon { width: 34px; height: 34px; }
    .beta-disclaimer-card h2 { margin-bottom: 18px; font-size: 1.42rem; }
    .beta-disclaimer-body { gap: 15px; }
    .beta-disclaimer-actions { margin-top: 22px; }
...
const selfTest = () => `
  ...
  const VECTORS = [
    ...
    ['SHA-256, 55-byte padding boundary', () => C.hex(C.sha256(new Uint8Array(55))),
  ...
  function prepareExport(kind) {
    ...
    try {
      ...
      $('export-status').textContent = isPrivate
        ? 'Recovery file download requested. Treat it like the wallet itself.'
        : 'Watch-only file download requested. It cannot spend, but it reveals wallet activity.';
    } catch (error) {
  ...
  <div class="wrap hero-shell">
    ...
    <div class="hero-heading">
      <div class="hero-copy">
        ...
        <div class="hero-meta">
          <a class="github-link" href="https://github.com/HodlDee/selfcustody/blob/main/docs/entropy-offline.html" target="_blank" rel="noopener noreferrer">
...
<div class="beta-disclaimer" id="beta-disclaimer" role="alertdialog" aria-modal="true" aria-labelledby="beta-disclaimer-title" aria-describedby="beta-disclaimer-copy beta-disclaimer-critical" hidden>
  <div class="beta-disclaimer-card">
    <div class="beta-disclaimer-icon">${warningTriangle('security-icon')}</div>
    <h2 id="beta-disclaimer-title">Before you use the Workshop</h2>
    <div class="beta-disclaimer-body">
      ...
      <div class="beta-disclaimer-divider" aria-hidden="true"></div>
    ...
    <div class="beta-disclaimer-actions">
      <div class="dl-frame">
        <button class="dl beta-disclaimer-accept" id="beta-disclaimer-accept" type="button"><span>I understand</span></button>
      </div>
    </div>
  </div>
</div>
...
<section class="results" id="results" hidden>
  ...
  <div class="export-grid">
    <article class="export-card is-private">
      <div class="export-card-heading">
        <span class="export-card-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><rect x="5" y="10" width="14" height="10" rx="2"/><path d="M8 10V7a4 4 0 0 1 8 0v3"/><path d="M12 14v2"/></svg></span>
        <div><span class="export-tag">Keep secret</span><h4>Private recovery</h4></div>
      </div>
      ...
      <div class="export-actions" role="group" aria-labelledby="export-download-label">
        <button type="button" class="export-button" id="export-private-open">Recovery file</button>
        <button type="button" class="export-button" id="export-wallet-open">wallet.dat</button>
      </div>
    </article>
    <article class="export-card is-watch">
      <div class="export-card-heading">
        <span class="export-card-icon" aria-hidden="true"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6S2.5 12 2.5 12Z"/><circle cx="12" cy="12" r="2.5"/></svg></span>
        <div><span class="export-tag">No spending keys</span><h4>Watch-only</h4></div>
      </div>
      ...
      <div class="export-actions" role="group" aria-labelledby="export-watch-download-label">
        <button type="button" class="export-button" id="export-watch">Watch-only file</button>
      </div>
    </article>
  </div>
  ...
  <dialog class="export-dialog" id="export-private-dialog" aria-labelledby="export-private-title">
    <form method="dialog">
      ...
      <p>The recovery file contains the recovery words, SeedQR digits and private keys shown above. The Bitcoin Core wallet.dat contains the selected account private key and its receiving and change descriptors. Anyone who gets either file can move the coins it controls.</p>
