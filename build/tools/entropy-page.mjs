...
const ui = () => `
  ...
  function paintMeter(input) {
    ...
    /* Shown past the target rather than pinned to it, because the reading is
       honest about the source: a hundred rolls really do carry 258.5 bits.
       What is not honest is leaving it there and letting a bar past full imply
       that more rolls keep helping. A seed holds what it holds -- everything
       above the target is hashed down into the same 256 bits. */
    const over = bits > target;
    $('meter-val').textContent = bits.toFixed(1) + ' / ' + target + ' bits'
      + (over ? ' \u2014 seed max ' + target : '');
    ...
    if (over) {
      $('meter-note').textContent += ' Past that, more ' + info.unit
        + 's change which wallet you get but not how hard it is to guess.';
    }
  }
