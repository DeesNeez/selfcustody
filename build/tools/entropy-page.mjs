...
${FONTS.map(embedFont).join('\n')}
  ...
  * { box-sizing: border-box; }
  ...
  .export-card.is-private { border-color: rgba(255, 138, 0, 0.34); background: rgba(255, 138, 0, 0.055); }
  .export-card.is-watch { border-color: rgba(53, 180, 138, 0.28); background: rgba(53, 180, 138, 0.045); }
  .export-tag {
    width: fit-content; margin-bottom: 8px; padding: 3px 7px; border-radius: 5px;
  ...
  .is-private .export-tag { color: #ffad4c; background: rgba(255, 138, 0, 0.11); }
  .is-watch .export-tag { color: #8be3c6; background: rgba(53, 180, 138, 0.1); }
  ...
  .is-private .export-button { color: #ffad4c; border-color: rgba(255, 138, 0, 0.36); }
  .is-watch .export-button { color: #8be3c6; border-color: rgba(53, 180, 138, 0.34); }
  ...
  .export-dialog {
    ...
    border: 1px solid rgba(255, 138, 0, 0.62); border-radius: 14px;
    box-shadow: 0 30px 80px rgba(0, 0, 0, 0.62);
  }
  .export-dialog::backdrop { background: rgba(8, 7, 6, 0.74); }
  .export-dialog h2 { margin: 0 32px 10px 0; color: #ffad4c; font-size: 1.25rem; }
  ...
  .export-dialog-close:hover { color: var(--orange); }
...
const ui = () => `
  ...
  const EXPORT_LABELS = {
    private: 'Download private record',
    watch: 'Download watch-only record'
  };
  ...
  function clearExportState() {
    ...
    const privateButton = $('export-private-confirm');
    const watchButton = $('export-watch');
    if (privateButton) {
      privateButton.disabled = false;
      privateButton.textContent = EXPORT_LABELS.private;
    }
    if (watchButton) {
      watchButton.disabled = false;
      watchButton.textContent = EXPORT_LABELS.watch;
    }
  }
  ...
  /* The builder receives the cached seed and only a boolean for passphrase
     ...
     activation; deferring it for a prettier "Preparing" repaint makes local
     file downloads more likely to be blocked as synthetic. */
  function prepareExport(kind, button) {
    if (!state.seed || $('results').hidden) return;
    const path = $('path').value.trim();
    button.disabled = true;
    button.textContent = 'Preparing\u2026';
    $('export-status').textContent = 'Preparing ' + (kind === 'private' ? 'private' : 'watch-only') + ' record\u2026';
    ...
    try {
      ...
      $('export-status').textContent = 'Could not prepare that record: ' + error.message + '.';
    } finally {
      button.disabled = false;
      button.textContent = EXPORT_LABELS[kind];
    }
  }
  ...
  $('export-private-confirm').addEventListener('click', event => {
    prepareExport('private', event.currentTarget);
  });
  $('export-watch').addEventListener('click', event => {
    prepareExport('watch', event.currentTarget);
  });
...
<section class="results" id="results" hidden>
  ...
  <p class="hint">Two plain-text files for two different jobs. The private one restores and spends; the watch-only one cannot spend, but still reveals the wallet&rsquo;s addresses and history.</p>
