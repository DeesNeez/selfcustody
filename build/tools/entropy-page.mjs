...
const ui = () => `
  ...
  function render() {
    ...
    /* 12 words are 48 digits and 24 are 96, which the vendored encoder puts in
       ...
       A length with no stated grid hides the fold rather than guessing one.
...
<section class="results" id="results" hidden>
  ...
  <div class="xpub-box xprv-box">
    ...
    <!-- Its own disclosure rather than a row inside the one above: a SeedQR is
         the recovery words as digits, not the account key, and filing it under
         that heading would teach the wrong thing in a tool whose job is
         telling these keys apart. -->
    <details class="xprv-more" id="seedqr-more">
      <summary><span class="xprv-more-head"><b>SeedQR recovery code</b><code id="seedqr-grid"></code><i aria-hidden="true"></i></span></summary>
      <div class="xprv-more-body">
        <p class="xpub-note">The recovery words as numbers: each word&rsquo;s place in the BIP39 list, four digits per word, in the order they appear above. <a href="https://github.com/SeedSigner/seedsigner/blob/dev/docs/seed_qr/README.md" target="_blank" rel="noopener noreferrer">SeedSigner</a> and compatible signing devices scan it. The heading says how many squares the code will be, so you know the size of the job before you start punching one into metal.</p>
